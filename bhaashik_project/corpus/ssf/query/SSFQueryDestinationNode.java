    ...
    //        p = Pattern.compile("(\\(?\\s*[^\\(\\)]+\\)?)\\s+AND\\s+(\\(?\\s*[^\\(\\)]+)", Pattern.CASE_INSENSITIVE);
        ...
        else if(args.length == 1)
        {
            ...
            if(destination.getLocation().equals(""))
            {
                ...
                int returnVal = chooser.showSaveDialog(BhaashikMain.getBhaashikMain());

                if(returnVal == JFileChooser.APPROVE_OPTION) {
                    destination.setLocation(chooser.getSelectedFile().getAbsolutePath());

                    destination.setCharset(JOptionPane.showInputDialog(BhaashikMain.getBhaashikMain(), GlobalProperties.getIntlString("Please_enter_the_charset:"), GlobalProperties.getIntlString("UTF-8")));
                }
                else {
                    return false;
                }
