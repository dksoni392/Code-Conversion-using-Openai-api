...
import bhaashik.corpus.ssf.impl.SSFCorpusAnalyzer;
...
import bhaashik.gui.JTextAreaCellEditor;
...
//    private SSFStory ssfUTF8Story[];
    ...
    protected BhaashikTableJPanel summaryTableJPanel;
    protected BhaashikTreeJPanel bhaashikTreeJPanels[];
    protected JPanel bhaashikTreeJPanel;
    protected BhaashikTableJPanel bhaashikTableJPanel;
    ...
    public SSFCorpusAnalyzerJPanel() {
        ...
        act.putValue(Action.MNEMONIC_KEY, Integer.valueOf(KeyEvent.VK_D));
...
//            treeJPanel.removeAll();
        ...
        if(compareLayoutJComboBox.getSelectedItem().equals(GlobalProperties.getIntlString("One_by_one")) == true) {
            ...
            if(jp == null)
            {
                treeJPanel.add(bhaashikTreeJPanel, BorderLayout.CENTER);

                if(propbankMode) {
                    extraInfoJPanel.setSSFPhraseJPanel((BhaashikTreeJPanel) bhaashikTreeJPanel);
                }
            }
            else
            {
                ...
                if(propbankMode) {
                    extraInfoJPanel.setSSFPhraseJPanel((BhaashikTreeJPanel) jp);
                }
            }
        ...
        {
            if(propbankMode) {
                extraInfoJPanel.setSSFPhraseJPanel((BhaashikTreeJPanel) bhaashikTreeJPanels[currentTaskIndex]);
            }
        }
...
//        initNavigationList();
    ...
    private void highlightWord() {
        ...
        for (int i = 0; i < tcount; i++)
        {
            ...
            BhaashikTreeJPanel ssfPhraseJPanel = bhaashikTreeJPanels[i];
...
//        editWordInfoExamples();
    ...
    private int getTreeMode(int compare_type) {
        if(compare_type == SSFCorpusAnalyzer.MM_TREES) {
            return BhaashikTreeJPanel.MM_TREE_MODE;
        }
        
        return BhaashikTreeJPanel.SSF_MODE;
    }
...
//            currentWPosition = Integer.parseInt(wpos2sposMap.getPropertyValue(currentPositionString)) - 1;
    ...
    private void displayDetail() {
        ...
        BhaashikTableModel comparisonTable = null;
        ...
        switch(currentCompareType) {
            ...
            case SSFCorpusAnalyzer.POS_TAGS:
                ...
                bhaashikTableJPanel = BhaashikTableJPanel.createTableDisplayJPanel(comparisonTable, langEnc);
                
                if(comparisonTableJPanel.getComponentCount() == 1) {
                    comparisonTableJPanel.remove(0);
                }
                
                comparisonTableJPanel.add(bhaashikTableJPanel, BorderLayout.CENTER);
            ...
            case SSFCorpusAnalyzer.COMMENTS:
                ...
                JTable jt = bhaashikTableJPanel.getJTable();
//		jt.setDefaultEditor()
    ...
    private JPanel fillDetailTreeJPanel(int taskInd) {
        ...
        if(getTreeMode(currentCompareType) == BhaashikTreeJPanel.MM_TREE_MODE) {
            bhaashikTreeJPanel = new BhaashikTreeViewerJPanel(tree, BhaashikMutableTreeNode.DEPENDENCY_RELATIONS_MODE, langEnc);
        } else
        {
            bhaashikTreeJPanel = BhaashikTreeJPanel.createSSFCompareJPanel(tree, phraseNamesPT, posTagsPT, langEnc);

            ((BhaashikTreeJPanel) bhaashikTreeJPanel).setPropbankMode(propbankMode);

            ((BhaashikTreeJPanel) bhaashikTreeJPanel).showControlTabs(false);
        }

        bhaashikTreeJPanel.setBorder(new TitledBorder(taskNames[taskInd]));
...
//            selecTaskJComboBox.setEnabled(true);
            ...
            {
                treeJPanel.add(bhaashikTreeJPanel, BorderLayout.CENTER);
                bhaashikTreeJPanels[0] = (BhaashikTreeJPanel) bhaashikTreeJPanel;
            }
            else
            {
                treeJPanel.add(jp, BorderLayout.CENTER);
                bhaashikTreeJPanels[0] = (BhaashikTreeJPanel) jp;
            }

            if(propbankMode)
            {
                ((SSFTreeCellRendererNew) bhaashikTreeJPanels[0].getJTree().getCellRenderer()).setMode(SSFTreeCellRendererNew.PROPBANK_ANNOTATION);
                bhaashikTreeJPanels[0].showControlTabs(false);
            }
            else {
                    ((SSFTreeCellRendererNew) bhaashikTreeJPanels[0].getJTree().getCellRenderer()).setMode(SSFTreeCellRendererNew.SYNTACTIC_ANNOTATION);
            }
...
//            int divlocation = (1/taskNames.length);
            ...
            for (int i = 0; i < taskNames.length; i++) {
                ...
                if(jp == null) {
                    bhaashikTreeJPanels[i] = (BhaashikTreeJPanel) bhaashikTreeJPanel;
                }
                else {
                    bhaashikTreeJPanels[i] = (BhaashikTreeJPanel) jp;
                }

                if(propbankMode)
                {
                    ((SSFTreeCellRendererNew) bhaashikTreeJPanels[i].getJTree().getCellRenderer()).setMode(SSFTreeCellRendererNew.PROPBANK_ANNOTATION);
                    bhaashikTreeJPanels[i].showControlTabs(false);
                }
                else {
                    ((SSFTreeCellRendererNew) bhaashikTreeJPanels[i].getJTree().getCellRenderer()).setMode(SSFTreeCellRendererNew.SYNTACTIC_ANNOTATION);
                }
                
                if(i == 0) {
                    if(jp == null)
                    {
                        jsp.setTopComponent(bhaashikTreeJPanel);
                    }
                    else
                    {
                    ...
                    if(jp == null) {
                        jsp.setBottomComponent(bhaashikTreeJPanel);
                    }
                    else {
...
//            int divlocation = (1/taskNames.length);
            ...
            for (int i = 0; i < taskNames.length; i++) {
                ...
                if(i == 0) {
                    if(jp == null) {
                        jsp.setLeftComponent(bhaashikTreeJPanel);
                    }
                    else {
                    ...
                    if(jp == null) {
                        jsp.setRightComponent(bhaashikTreeJPanel);
                    }
                    else {
...
//            selecTaskJComboBox.setEnabled(false);
    ...
    public void compare() {
        ...
        BhaashikTableModel summaryComparisonTable = ssfPhraseComparison.comparisonSummary();
        summaryTableJPanel = BhaashikTableJPanel.createTableDisplayJPanel(summaryComparisonTable, langEnc);
...
//        PropertiesTable tasks = owner.getTaskList();
        ...
        bhaashikTreeJPanels = new BhaashikTreeJPanel[taskNames.length];
