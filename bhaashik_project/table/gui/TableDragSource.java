...
public class TableDragSource implements DragSourceListener, DragGestureListener {
  ...
  protected BhaashikJTable sourceTable;

  protected int mode;

  protected boolean dragStarted = false;

  public TableDragSource(BhaashikJTable table, int actions, int mode) {
    sourceTable = table;
  ...
  public void dragGestureRecognized(DragGestureEvent dge) {
      ...
      if(dragObject instanceof SSFNode || dragObject instanceof AlignmentUnit)
      {
        ...
        transferable = new BhaashikTransferableObject(dragObject);
  ...
  public void dragDropEnd(DragSourceDropEvent dsde) {    
    ...
    if (dsde.getDropSuccess())
    {
        if(mode == BhaashikJTable.TREE_MODE)
        {
            if(dsde.getDropAction() == DnDConstants.ACTION_MOVE)
            {
                ...
                ((BhaashikTreeModel) sourceTable.getObjectModel()).removeNodeFromParent((DefaultMutableTreeNode) oldDragObject);

                if(oldNodeParent.getChildCount()==0)
                    ((BhaashikTreeModel) sourceTable.getObjectModel()).removeNodeFromParent(oldNodeParent);
    ...
    //            ((BhaashikTreeModel) sourceTable.getObjectModel()).removeNodeFromParent(oldNode);
    //
    //            if(oldNodeParent.getChildCount()==0)
    //                ((BhaashikTreeModel) sourceTable.getObjectModel()).removeNodeFromParent(oldNodeParent);
